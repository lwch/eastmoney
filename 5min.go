...
func (t *dateTime) UnmarshalJSON(data []byte) error {
	tm, err := time.ParseInLocation(time.DateTime, string(data[1:len(data)-1]), time.Local)
	if err != nil {
		return err
	}
	*t = dateTime(tm)
...
// KLine5Min 获取5分钟K线数据
func (cli *Client) KLine5Min(code string, from, to time.Time, right right) ([]Tick, error) {
